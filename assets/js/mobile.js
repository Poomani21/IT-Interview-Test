...
$(document).ready(function () {
  ...
  $.getJSON("assets/data/ios_questions.json", function (data) {
    questions = data;
...
function createNavigation() {
  ...
  for (let i = 0; i < filtered.length; i++) {
    ...
    if (answeredQuestions[q.id]) {
      if (answeredQuestions[q.id].selected !== null || answeredQuestions[q.id].showAnswer) {
...
//   return filtered.every(q => answeredQuestions[q.id]);
// }
function allQuestionsAnswered() {
